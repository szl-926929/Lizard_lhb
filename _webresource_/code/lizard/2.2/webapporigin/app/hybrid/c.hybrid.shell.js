 ...
 * 1. 对于 H5 中尚未对等实现的功能（如调取系统联系人），我们将通过 cHybridShell 对 bridge.js 中的方法提供二次封装；  
 * 2. 对于 H5 中已经对等实现的功能（如定位），我们将通过 Guider 等功能类提供二次封装；  
 * 更多细节请参考@see http://conf.ctripcorp.com/display/FRAM/cHybridShell
 * @namespace cHybridShell
...
define([], function() {
    ...
    HYBRID.CLASSES = [];
    for (var i in W) {
      if (i.substr(0, 5) == 'Ctrip') {
        HYBRID.CLASSES.push(W[i]);
      }
    }
    // var HYBRID.CLASSES = [
    //  CtripBar            ,
    ...
    //  check_network_status               : { paramsMixed: true },  // 反馈信息是否混淆在回调参数中。
        ...
        sotp_send_http_requst              : { sidIndex: 4 },  // sequenceId 在形参中的位置，顺序自 0 起始。
    ...
    HYBRID.fninfo = function(tagname) {
        ...
        if (!info._READY) {
            ...
            if (!info.hasOwnProperty('klass')) {
              info.klass = _.find(HYBRID.CLASSES, function(klass) { return !!klass[info.realname]; });
            }
            // 信息初始化不会反复尝试。
            info._READY = true;
        }
    ...
    var _ME = {
        ...
        SN: {
            ...
            gen     : (function() {
              var cursor = 1;
              return function() { return (new Date()).getTime() + '.' + cursor++; };
            })()
        },
        ...
        apiFn: function(tagname) {
            ...
            if (info.klass) {
                method = function() {
                  ...
                  abort = _.find(_ME.fn('find', tagname, _ME.SN.PRE), function(fn) {
                    ...
                    info.klass[info.realname].apply(info.klass, args);
                };
            }
            return method;
        },
    ...
    var exports = {
        ...
        Fn: function(T, callback) {
              if (this === exports) {
                return new this.Fn(T, callback);
              }
              // 拥有序列号参数，意味着该方法支持并发。
              var tagname = T, sequenceId, sidIndex = HYBRID.fninfo(tagname).sidIndex;

              // 自动生成序列号
              if (_.isNumber(sidIndex)) {
                sequenceId = _ME.SN.gen();
              }
              ...
              this.run = function() {
                  ...
                  if (_.isNumber(sidIndex)) {
                      ...
                      for (var i = 0, n = Math.max(sidIndex + 1, arguments.length); i < n; i++) {
                          if (i == sidIndex) {
                            args.push(sequenceId);
                          }
                          args.push(arguments[i]);
                      }
                  }
                  if (window.app.__callbackqueue && !Lizard.hybridready) {
                    window.app.__callbackqueue.push(function(){

(function () {
  ...
  ****/
  ...
  window.app = {};
  function setCommonStore(CommonStore, params) {
    ...
    if (wStore && params) {
      ...
      //以下信息放到内存中，在onshow的时候清理掉
      if(params.networkStatus){
        Lizard.networkStatus = params.networkStatus;
        require(["cHybridShell"],function(cHybridShell){
          cHybridShell.preTreat('check_network_status', function() {
            if (Lizard.networkStatus) {
              window.app.callback({ tagname: 'check_network_status', param: {
                hasNetwork: true,
                networkType:  Lizard.networkStatus}});
              cHybridShell.abort();
            }
          });
        });
      }
      if(params.ctripCity && params.geoAddress){
        var locationObj = {
          timeout:new Date().getTime()
        };
        locationObj.ctripCity =params.ctripCity;
        locationObj.geoAddress =params.geoAddress;
        Lizard.locationObj = locationObj;
      }
  ...
  if (params && params.versionCode && params.version) {
    window.app.__callbackqueue = [];
    var timeout = Lizard.isHybrid ? 2000 : 4000;
    require(['libs', 'cCommonStore'], function (libs, CommonStore) {
      setCommonStore(CommonStore, params);
      mutileLoad();
      setTimeout(function(){
        if (window.app.__callbackqueue) {
          Lizard.hybridready = true;
          _.each(window.app.__callbackqueue, function(func){
            func();  
          });
          window.app.__callbackqueue = [];
        }
      ...
      var methods = {
        'web_view_finished_load': function () {
          ...
          if (!Lizard.config.isBeforeViewFinishedLoadInitApp && Lizard.config.isViewFinishedLoadInitApp) {
            require(['cHybridGuider'], function(Guider){
              Guider.create();
              mutileLoad();
            });            
          }
        ...
        'init_member_H5_info': function (params) {
          ...
          require(['libs', 'cCommonStore', 'cHybridGuider'], function (libs, CommonStore, Guider) {
            setCommonStore(CommonStore, params);
            if (Lizard.app.vendor.is('CTRIP') && !Lizard.config.isBeforeViewFinishedLoadInitApp && !Lizard.config.isViewFinishedLoadInitApp) {
              Guider.create();
              mutileLoad();
            }
            if (Lizard.config.isBeforeViewFinishedLoadInitApp) {
              require(["cHybridGuider"], function(e){e.create();});
            }
          });
        },

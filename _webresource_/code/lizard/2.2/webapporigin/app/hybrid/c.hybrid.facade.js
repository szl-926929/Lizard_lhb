    ...
    LOCATE: {
      ...
      parseCallback: function (options) {
        return function (params, err) {
          var errcode;
          if (err) {
            errcode = err.error_code || err.number;
          }
          try {
  ...
  var exports = {
    ...
    init: function () {
      ...
      pretreater = function (log, result) {
        if (!window.localStorage.getItem('isPreProduction')) {
          cHybridShell.abort();
        }
      ...
      callback = function (params) {
        ...
        Lizard.networkQuality = params.networkQuality;
      };
      cHybridShell
      ...
      posttreater = function (params, err) {
        ...
        if (err) {
          params.error_code = err.number;
        }
        return params;
      };
      cHybridShell.postTreat('save_photo', posttreater);
      if (window.app.__callbackqueue) {
        cHybridShell.once('web_view_finished_load', function(appInfo){
    ...
    register: function (options) {
      ...
      var tagname = '';
      try {
        tagname = _ME.var2tagname(options.tagname);
      } catch (ex) {
        return;
      }  // 在旧版中，遇此类情况不作为，不报错！
      if (!tagname) {

  ...
  return cCoreInherit.Class(APP, {
    bindEvents: function ($super) {
      ...
      $(window).bind('popstate', _.bind(function (e) {
        ...
        if ($.os.ios && Lizard.app.code.is('WEIXIN') && ( (1==history.length)|| (this.__isLanding && e.state && e.state.start))) {
          return;
        }
    ...
    _loadViewByLocationUrl: function()
    {
      ...
      if (!Lizard.app.vendor.is('CTRIP')) {
        history.replaceState({url: landingpath, text: document.documentElement.innerHTML, options: {pushState: false, renderAt: 'server', landingpage: 1, hideloading: Lizard.config.isFirstPageHideLoading},start:1}, document.title, landingpath);
      }
    ...
    goTo: function (url, opt) {
      ...
      if (UtilCommon.isUrl(url) || url.indexOf('http://localhost') === 0) {
        var indexwebapp = url.indexOf('/webapp'), indexhtml5 = url.indexOf('/html5');
        if (indexwebapp === -1) {
          url = url.substr(indexhtml5);  
        } else if (indexhtml5 === -1) {
          url = url.substr(indexwebapp);  
        } else {
          url = url.substr(indexhtml5 > indexwebapp?indexwebapp:indexhtml5); 
        }  
      }
    ...
    _loadViewByOptions: function(url, html, opt)
    {
      ...
      if (opt && opt.animBackwardName) {
        this.animBackwardName = opt.animBackwardName;
      }

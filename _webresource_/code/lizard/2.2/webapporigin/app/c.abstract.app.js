 ...
 * @version V2.1
 */
define(['cPageModelProcessor', 'cUtilPerformance', 'cUtilCommon', 'cMessageCenter', 'UIAnimation', 'UILoading', Lizard.app.version.gte(6.4)? 'cHybridShell' : '','cPageParser'],
  function (callModels, cperformance, utils, MessageCenter, animation, UILoading, cHybridShell) {
    ...
    APP.prototype = {
      ...
      initialize: function initialize(options) {
        ...
        require(['UIAlert'], _.bind(function(UIAlert){
          this._alert = new UIAlert();
          this._confirm = new UIAlert();
        }, this));
        require(['UIToast'], _.bind(function(UIToast){
          this._toast = new UIToast();
        }, this));
        require(['UILoadingFailed'], _.bind(function(UILoadingFailed){
          this._warning404 = new UILoadingFailed();
        }, this));
        require([((Lizard.app.vendor.is('CTRIP') || Lizard.isHybrid) && !Lizard.app.code.is('WE')) ? 'cHybridHeader' : 'UIHeader'], _.bind(function(UIHeader) {
          this.headerView = new UIHeader({ 'root': $('#headerview'),'parent':this });
          if (Lizard.isHybrid) {
      ...
      showHisCtnrView: function (onShow, onHide, options) {
        ...
        if (!options || options.addToHistory !== false) {
          if (Lizard.isHybrid) {
            ...
            if (navigator.userAgent.indexOf("baidubrowser") > -1 && $.os.ios) {
              url += config.viewName;
            }

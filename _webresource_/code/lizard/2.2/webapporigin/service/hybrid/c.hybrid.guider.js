 ...
 */
define(['cHybridFacade', 'cHybridShell','cUtilPath','cUtilPerformance'], function (Facade, Hish,Path,cperformance) {
  "use strict";

  var HybridGuider = /** @lends cGuiderService */ {
    /**
     * 打开链接URL地址，兼容App和浏览器
     * @param {object} options 输入参数
     * @param {string} options.url 跳转的url
     * @example
     * 参数：
     * {
      ...
      var model = {
        refresh: function () {
          cperformance.log({
            ...
            fun: "cHybridGuider.jump.refresh"
          });
        ...
        browser: function () {
          cperformance.log({
            name: 'FunUsed',
            url: window.location.href,
            fun: "cHybridGuider.jump.browser"
          });
          if (options && options.url) {
        ...
        open_relative: function () {
          cperformance.log({
            name: 'FunUsed',
            url: window.location.href,
            fun: "cHybridGuider.jump.open_relative"
          });
          if (options && options.url) {
    ...
    apply: function (options) {
	  ...
	  /**
	   * 进入H5模块，初始化数据 H5接收到web_view_did_finished_load的回调之后，调用该函数，初始化数据会通过callback传递给H5
	   * @param {object} options 输入参数
     * @param {function} options.callback 回调
     * @param {string} options.version app版本号
	   * @example
	   * guiderService.init({
	   *   version:5.8，
     *   callback:function(){}
    ...
    init: function (options) {
	  ...
	  /**
	   * 上传日志至服务端, H5页面调用该函数，需要将增加的event_name告知native，native需要整理纪录
	   * @param {object} options
     * @param {string} options.name 日志
	   * @example
	   * //参数：
	   * {name:""}
	   */
     ...
     * @param {string} [options.result] 上一句log执行的结果，可以为空,打印的时候会自动换行，加入时间
	   * @example
	   * guiderService.print({log:"",result:""})
	   */
    print: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.print"
      });
    ...
    backToLastPage: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.backToLastPage"
      });
    ...
    addWeixinFriend: function () {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.addWeixinFriend"
      });
    ...
    showNewestIntroduction: function () {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.showNewestIntroduction"
      });
      Facade.request({ name: Facade.METHOD_SHOW_NEWEST_INTRODUCTION });
    },

	  /**
	   * 自定义注册需要hybrid回调执行的方法（确保此方法会在hybrid中执行，不然不会执行回调函数
	   * @param {Object} options 输入参数
    ...
    checkAppInstall: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.checkAppInstall"
      });
    ...
    refreshNative: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.refreshNative"
      });
    ...
    copyToClipboard: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.copyToClipboard"
      });
    ...
    readFromClipboard: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.readFromClipboard"
      });
    ...
    shareToVendor: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.shareToVendor"
      });
    ...
    downloadData: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.downloadData"
      });
    ...
    chooseContactFromAddressbook: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.chooseContactFromAddressbook"
      });
    ...
    hideLoadingPage: function () {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.hideLoadingPage"
      });
    ...
    showLoadingPage: function () {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.showLoadingPage"
      });
    ...
    choose_invoice_title: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.choose_invoice_title"
      });
    ...
    get_device_info: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.get_device_info"
      });
    ...
    show_voice_search: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.show_voice_search"
      });
    ...
    choose_photo: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.choose_photo"
      });
    ...
    finished_register: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.finished_register"
      });
    ...
    app_call_system_share: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_call_system_share"
      });
    ...
    app_check_network_status: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_check_network_status"
      });
    ...
    app_check_android_package_info: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_check_android_package_info"
      });
    ...
    app_log_google_remarkting: function (url) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_log_google_remarkting"
      });
    ...
    app_read_verification_code_from_sms: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_read_verification_code_from_sms"
      });
    ...
    app_h5_page_finish_loading: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.app_h5_page_finish_loading"
      });
    ...
    registerAppearEvent: function (callback) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.registerAppearEvent"
      });
    ...
    unregisterAppearEvent: function () {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.unregisterAppearEvent"
      });
  ...
  /**
   * @description 文件操作相关类
   * @namespace cGuiderService.file
   */
  HybridGuider.file = /** @lends cGuiderService.file */{
    /**
    ...
    isFileExist: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.isFileExist"
      });
    ...
    deleteFile: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.deleteFile"
      });
    ...
    getCurrentSandboxName: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.getCurrentSandboxName"
      });
    ...
    getFileSize: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.getFileSize"
      });
    ...
    makeDir: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.makeDir"
      });
    ...
    readTextFromFile: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.readTextFromFile"
      });
    ...
    writeTextToFile: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.file.writeTextToFile"
      });
  ...
  /**
   * @description 管道相关类
   * @namespace cGuiderService.pipe
   */
  HybridGuider.pipe = /** @lends cGuiderService.pipe */{
    /**
    ...
    socketRequest: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.pipe.socketRequest"
      });
    ...
    httpRequest: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.pipe.httpRequest"
      });
      var timestamp = Date.now();
    ...
    abortRequest: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.pipe.abortRequest"
      });
  ...
  HybridGuider.pay = {
    ...
    checkStatus: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.pay.checkStatus"
      });
   ...
   * @namespace cGuiderService.encrypt
   */
  HybridGuider.encrypt = /** @lends cGuiderService.encrypt */{
    ...
    ctrip_encrypt: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.encrypt.ctrip_encrypt"
      });
    ...
    ctrip_decrypt: function (options) {
      cperformance.log({
        name: 'FunUsed',
        url: window.location.href,
        fun: "cHybridGuider.encrypt.ctrip_decrypt"
      });

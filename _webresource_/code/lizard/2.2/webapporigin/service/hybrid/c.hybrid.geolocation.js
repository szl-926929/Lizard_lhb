...
define(['cUtilPerformance', 'cHybridFacade'], function (cperformance, Facade) {
   ...
   */
  Geo.requestCityInfo = function (callback, error, posCallback, posError, isAccurate, cityCallBack, cityErrorCallBack, isForceLocate) {
    var uuidGeoRequest = {
    ...
    try{
      var locationObj = Lizard.locationObj;
      if(locationObj && locationObj.geoAddress && locationObj.ctripCity){
        var geoAddress = locationObj.geoAddress;
        if (typeof posCallback == 'function') {
          posCallback(geoAddress.lng, geoAddress.lat);
        }
        if (typeof callback == 'function') {
          ...
          callback({
            lng: geoAddress.lng,
            lat: geoAddress.lat,
            city: geoAddress.city || geoAddress.ctyName || geoAddress.province,
            province: geoAddress.province,
            district: geoAddress.district,
            //+2……2014-09-04……JIANGJing
            country: geoAddress.country,
            countryShortName: geoAddress.countryShortName,
            address: geoAddress.addrs,
            detailAddress:geoAddress.detailAddress,
            pois:geoAddress.pois
          });
        }
        if(typeof cityCallBack == 'function'){
          cperformance.groupEnd(uuidGeoRequest.city);
          cityCallBack(locationObj.ctripCity);
        }
        return;
      }
    }catch(e){}

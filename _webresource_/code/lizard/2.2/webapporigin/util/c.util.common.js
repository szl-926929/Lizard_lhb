 ...
 * @alias cUtility
 */
define(['cUtilDate','cUtilPath','cUtilPerformance'],function (cDate,cPath,cperformance) {
  var Util = {};
   ...
   * @type {boolean}
   */
  Util.isPrivateModel = Lizard.isPrivateModel;
   ...
   * 获取obj的length
   */
  Util.getObjLength = function(obj){
    var str = "",res = 0;
    try{
      ...
      for(var i= 0,len = str.length,charCode = -1;i<len;i++){
        charCode = str.charCodeAt(i);
        if (charCode >= 0 && charCode <= 128){ res += 1;}
        else{ res += 2;}
      }
    }catch(e){

      ...
      log: function(opt, value) {
        ...
        if(opt.name == "FunUsed"){
          try{
            ...
            var isPreProduction = window.localStorage.getItem("isPreProduction");
            var sendList = window.localStorage.getItem("LizardFunUbtList");
            var isAlreadySend = false;
            if(sendList && sendList.indexOf(tags.fun) != -1){ // jshint ignore:line
              isAlreadySend = true;
            }
            if(((/^https?/.test(origin) && origin.indexOf("uat") != -1) || (isPreProduction == 1)) && !isAlreadySend){
              //uat环境或者堡垒包中使用
              this.send(def.name, tags, value, this.getTime());
              sendList = !sendList ? tags.fun : sendList + "," + tags.fun; // jshint ignore:line
              window.localStorage.setItem("LizardFunUbtList",sendList);
            }
          }catch(e){

  ...
  function(cCoreInherit, cModel, CommonStore, cUtilCommon) {
    ...
    UserModel.NotUserLoginModel = new cCoreInherit.Class(cModel, /** @lends cUserModel.NotUserLoginModel.prototype */{
      __propertys__: function() {
        this.url = "/H5Login/Account/NonUserLogin";
        this.param = {};
      ...
      excute: function(onComplete, onError, ajaxOnly, scope, onAbort) {
        ...
        var url = ["https://",domain,this.url].join("");
        ...
        var params = {
          clinetId:cUtilCommon.getGuid()
        };
        //唉 为了兼容老的使用方式，暂时不使用abstractModel中的方法，所以这里自己实现
        //todo 改成通用model中的方式
        var reqObj = {
          'type': 'post',
          'url': url,
        ...
        try{
          ...
          if(e && e.message == "INVALID_STATE_ERR: DOM Exception 11"  && window.XMLHttpRequest){
            ...
            xhrObj.onreadystatechange = function () {
              if (xhrObj.readyState == 4) {
                ...
                if (status >= 200 && status < 300) {
                  if(reqObj.success){
                    reqObj.success($.parseJSON(xhrObj.responseText), xhrObj.responseXML);
                  }
                } else {
                  if(reqObj.error){
                    reqObj.error(status);
            ...
            xhrObj.open(reqObj.type, reqObj.url, true);
    ...
    UserModel.ThirdPartInfoModel = new cCoreInherit.Class(cModel,  /** @lends cUserModel.ThirdPartInfoModel.prototype*/ {
      ...
      seturl:function(){
        ...
        }else if (host.match(/(\.ui\.|\.dev\.|\.fws|\.fat|\.lpt|localhost|172\.16|127\.0)/i)){

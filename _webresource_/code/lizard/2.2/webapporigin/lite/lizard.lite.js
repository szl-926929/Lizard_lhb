...
(function(window, document){
  ...
  var cAjax = (function(){
    ...
    function _sendReq(opt) {
      ...
      try{
        ...
        if(/^https?/.test(origin) && (arr[2] && arr[2].indexOf(origin) === -1) && containsDomain  && opt.dataType != "jsonp"){
  ...
  var AbsModel = function(){
    var model = {
      ...
      protocalExten:{
        name:"protocal",
        value:(function(){return window.location.protocol.match(/(http|https|file):/g)[0].replace(":","") })()
      }
    };
    model.execute=function(onComplete, onError,scope,onAbort){
      //组织请求头部
      ...
      try{
        params.head.extension = utils.extend(true,[],this.extension);
        params.head.extension.push(this.protocalExten);
      }catch(e){
        console.log("entension not Array");
      }
  ...
  Member.prototype = {
    ...
    getDuid:function(){
      return utils.getCookie("DUID");
    },
    isNonUser: function (callback) {
      return  Lite.UserStore.isNonUser(callback);
    },
    isLogin: function (callback) {
      return  Lite.UserStore.isLogin(callback);
    },
    isLoginWithoutType:function(callback){
      return  Lite.UserStore.isLoginWithoutType(callback);
    },
  ...
  Lite.NotUserLoginModel = function(suc,err,scope,params){
    var url = 'https://' + utils.getDomain("accounts") + "/H5Login/Account/NonUserLogin";
    var opts = {
      method:"get",
      timeout:25000
    };
    ...
    return this.ajax = cAjax.cros(url, opts.method, params || {}, successCallback, errorCallback,opts.timeout);
  };
  ...
  Lite.UserStore = (function(){
    var userStore = {
      ...
      isLogin: function (callback) {
        ...
        var cookieDuid = utils.getCookie("DUID");
        var cookieIsNonUser = utils.getCookie("IsNonUser");
        var isLogin =  (!!cookieDuid && cookieIsNonUser != "T") || (user && !!user.Auth && !user.IsNonUser);
      ...
      isNonUser: function (callback) {
        ...
        var cookieDuid = utils.getCookie("DUID");
        var cookieIsNonUser = utils.getCookie("IsNonUser");
        var isNonUser = (!!cookieDuid && cookieIsNonUser == "T") || (user && !!user.IsNonUser);
        if(typeof callback == "function"){
          //如果有回调函数，则使用回调函数强制获取用户信息
          callback(isNonUser);
        }else{
      ...
      isLoginWithoutType:function(callback){
        var user = this.getUser();
        var cookieDuid = utils.getCookie("DUID");
        var isLoginWithoutType = (!!cookieDuid) || (user && !!user.Auth); //只要有auth即可以判断登录，或者cookie中存入了DUID也是可以判断登录
        if(typeof callback == "function"){
  ...
  Lite.Utils = utils;
  Lite.__callbackQueue = {
    ready:[],
    hybridReady:[]
  };
  //hybrid或者直连情况下，走该逻辑
  Lite.instance = false;
  Lite.hybridInstance = false;
  Lite.ready = function(callback){
    if(Lite.instance){
      callback();
      return;
    }
    Lite.__callbackQueue.ready.push(callback);
  };
  Lite.HybridReady = function(callback){
    if(Lite.hybridInstance){
      callback();
      return;
    }
    Lite.__callbackQueue.hybridReady.push(callback);
  };
  function handleInstanceCallback(){
    if( Lite.instance){
      var j = null;
      while(j=Lite.__callbackQueue.ready.shift()){
        try{
         j();
        }catch(e){}
      }
    ...
    if(Lite.hybridInstance){
      var r = null;
      while(r = Lite.__callbackQueue.hybridReady.shift()){
        try{
          r();
        }catch(e){}
      }
  ...
  //加个保险，4s之后所有回调都执行
  setTimeout(function(){
    if(!Lite.instance || !Lite.hybridInstance){
      Lite.instance = true;
      Lite.hybridInstance = true;
      handleInstanceCallback();
    }
  },4000);
  //判断lite运行环境
  function appInfo() {
    ...
    return {
      ...
      vendor : {
        ...
        is : function (vendor) {
          if ((Lite.app.code.is('TV')|| Lite.app.code.is('TY')) && vendor == 'CTRIP') {
            return false;
          }
  ...
  Lite.app = appInfo();
  if(Lite.app.vendor.is("CTRIP") && typeof  Lizard == "undefined"){
    (function(){
      ...
      function handleParam(params){
        ...
        if (params.userInfo) {
          try {
            ...
            Lite.HeadStore.setAuth(params.userInfo.data.Auth);
            Lite.UserStore.setUser(params.userInfo.data);
          } catch (e) {
        ...
        if (params.sourceId) {
          Lite.HeadStore.setAttr("sid",params.sourceId);
        ...
        //接受clientId,供UBT使用
        if (params.clientID) {
          Lite.HeadStore.setAttr("cid",params.clientID);
          window.localStorage.setItem('GUID', params.clientID);
        }
        //外部渠道号
        if (params.extSouceID) {
          Lite.HeadStore.setAttr("xsid",params.extSouceID);
        }
        //hybrid下直接接受App传递的参数,标准版-iOS:12, android:32; 学生版－ios:16, Android:36, since 5.6
        if (params.systemCode) {
          Lite.HeadStore.setAttr("syscode",params.systemCode);
        }
        if (params.version) {
          Lite.HeadStore.setAttr("cver",params.internalVersion || params.version);
        }
        ...
        //保存原始参数值
        window.localStorage.setItem('CINFO',JSON.stringify(params));
      }
      //hybrid下不跟lizard主板冲突！lizard主板会进行初始化，直接取就可以了
      var params = null;
      try {

/**
 * Created by bianyl on 2016/5/26.
 */
(function(LizardLite){
  LizardLite._query_weixinReady = [];
  LizardLite.weixinReady = function(callback){
    if(LizardLite.weixinShell){
      callback(LizardLite.weixinShell);
      return;
    }
    LizardLite._query_weixinReady.push(callback);
  };
  var handleWinXinShell = function(){
    var f = null;
    while(f = LizardLite._query_weixinReady.shift()){
      typeof f == 'function' && f(LizardLite.weixinShell);
    }
  };
  if(typeof Lizard != "undefined" && typeof require == "function"){
    require(["cShell"],function(cShell){
      LizardLite.weixinShell = cShell;
      handleWinXinShell();
    })
  }else{
    if(typeof require == 'function'&& typeof define == 'function' && (define.amd || define.cmd)){
      require(["//res.wx.qq.com/open/js/jweixin-1.0.0.js"],function(jWeixin){
        handleJsSdkDownload(jWeixin);
      });
    }else{
      ...
      s.onload = function(){
        if(typeof wx !== "undefined"){
          handleJsSdkDownload(wx);
        }else{
          console.log("微信js加载失败！");
        }
      };
      s.onerror = function() {
        console.log("微信js加载失败！");
    ...
    function handleJsSdkDownload(wx){
      if(typeof wx !== "undefined"){
        var vendor = initWeiXin(wx);
        vendor.ready(function(){
          LizardLite.weixinShell = handleMethodsToPromis(vendor.METHOD);
          handleWinXinShell();
        }).error(function(){
          handleWinXinShell();//也会执行，但是可能执行的结果都是undefined的
        }).init();
      }
    }

    function initWeiXin(jWeixin){
      var
      ...
      // 获取自定义公众号
      var mete = document.querySelector('meta[name=weixinAppId]');
      var _AUTONOMOUS_APP_ID = mete && mete.content;
      ...
      var _ME = {
        ...
        available: (function() {
          ...
          if (LizardLite.app.code.is('WEIXIN') && LizardLite.app.version.lt('6.0.2')) { availables = {};}
          ...
          var judge = function(all /*, OPTIONAL Function onsuccess, OPTIONAL Function onerror, String fnName_1, OPTIONAL String fnName_2, ...*/) {
            ...
            if (typeof arguments[i] == 'function') {
              ...
              if (typeof arguments[i] == 'function') {onerror = arguments[i++];}
            }

            if (Object.prototype.toString.call(arguments[i]) === '[object Array]') { i = 0; fns = arguments[i];}
            else { fns = arguments;}

            init(function() {
              for (; i < fns.length; i++) {
                var tpavailable = !!availables[fns[i]];
                if ( tpavailable != all) {
                  ret = !all;
                  break;
                }
      ...
      var exports = {
        ...
        METHOD: {
          ...
          voice2text: {
            run: function(onsuccess, onerror, options) {
              options = LizardLite.Utils.extend({
    ...
    function handleMethodsToPromis(methods){
      ...
      function doCall(name,type,args){
        ...
        var P = {},
          ...
          funcargs = [ ondone, onfail, options ];
        var tempObj = methods[name];
        var tempMethod = tempObj && tempObj[type];
        if(typeof tempMethod == 'function'){
          tempMethod.apply(null, funcargs);
        }
        return P;
      }
...
// 创建一个方法函数。
      function _Fn(method){
        // 方法主体函数
        function Fn() {
          var args = method.pre? method.pre.apply(null, arguments) : arguments;
          return doCall(method.name, 'run', args);
        }
        // 附属可用性判别函数，支持链式表达式。
        Fn.ping = function() {
          return doCall(method.name, 'ping', arguments);
        };
        return Fn;
      }
      var exports = {};
      for(var j= 0,l=_METHOD.length;j<l;j++){
        var method = _METHOD[j];

  ...
  function _sendReq(opt) {
    ...
    var obj = {
      ...
      success: function (res, status, xhr) {
        ...
        cperformance.performance[uuidXhrSend].info = ['code:', xhr && xhr.status || 200].join();
        if (_.isObject(res) && res.ResponseStatus && res.ResponseStatus.Extension) {
          var logtrace = _.find(res.ResponseStatus.Extension, function(ext){
            return ext.Id === "CLOGGING_TRACE_ID";
          });
          if (logtrace) {
            cperformance.performance[uuidXhrSend]["CLOGGING_TRACE_ID"] = logtrace.Value; 
          }
        }
      ...
      error: function (err, status) {
        if (status !== 'abort') {
          ...
          if (err && _.isObject(err)) {
            cperformance.performance[uuidXhrSend].info = ['code:', err.status, '    res:', encodeURIComponent(err.responseText)].join();
          }
          cperformance.groupEnd(uuidXhrSend);
        }
        if (opt.error) {

...
define(['cHybridShell', '_', '$'], function(Hish) {
    ...
    var _ME = {
        ...
        loadJS: (function() {
            ...
            return function(/*String*/ src, /*OPTIONAL Function*/ callback) {
                ...
                if (!script) {
                    ...
                    document.write = _Agent.documentWrite;  // jshint ignore:line
                    document.write('<script src="' + src + '"><\/script>');  // jshint ignore:line
                    document.write('', null, function() {  // jshint ignore:line
                        script.loaded = 1;
        ...
        overload: (function() {
            ...
            var overload = function(/*Array[Array]*/ definitions, /*Object ARGUMENTS*/ args, /*Object POINTER*/ scope) {
                ...
                _.find(definitions, function(def) {
                    ...
                    if (match) {
                        return ret = fn.apply(scope, args), true;
                    }
                });
        ...
        time2ms: function foo(time, defaultValue) {
            ...
            if (time != window.Infinity) {
                time = window.parseInt(time);
            }

            if (window.isNaN(time)) {
                return defaultValue ? foo(defaultValue) : 0;
            }
            else {
                return time;
            }
        ...
        documentWrite: (function() {
            ...
            var 
                ...
                extractNodes = /*Array(Node)*/ function(/*Element*/ node) {
                    ...
                    while (subnode = node.childNodes[0], subnode) {
                ...
                syncWrite = function foo(/*Element*/ container, /*Array*/ nodes, /*OPTIONAL Function*/ callback) {          
                    ...
                    var _vallbackfunc = function() {
                            foo(container, nodes, callback);
                        };
                    // 将节点从数组中依 FIFO 顺序移出。
                    while (node = nodes.shift(), node) {
                        ...
                        if (subnodes) {
                            foo(node, subnodes, _vallbackfunc);
                        }
            ...
            return function foo(/*String*/ html, /*OPTIONAL Element*/ container, /*OPTIONAL Function*/ callback) {
                ...
                if (dwstatus) {
                    ...
                    if (!heap.length || currentScript != lastScript) {
                        heap.unshift([]);
                    }
                    ...
                    if (currentScript) {
                        snippet.container = currentScript.parentNode;
                    }
    ...
    var I_Deferred = function(promiseTo) {
        var I = this, handlers = {};
    ...
    var _HOST_UA = {
                ...
                var 
                    ...
                    watchId = geo.watchPosition(
                        ...
                        function(position) {
                            cancel();
                            if (onsuccess) {
                                onsuccess(new Coordinate(position));
                            }
                        },
                        ...
                        function(err) {
                            cancel();
                            _ME.onerror(onerror, I_ERROR.Position(err));
                        },
        ...
        address: function(onsuccess, onerror, options) {
            ...
            // 创建计时器
            var T = new I_Timer(options.timeout);
            ...
            scaffold: /*Function*/ function(/*String*/ klassName, /*OPTIONAL Array(String)*/ methodNames, /*OPTIONAL Function*/ onTact) {
                ...
                defines[klassName] = function() {
                    ...
                    if (klasses[klassName]) {
                        return construct(klasses[klassName], arguments);
                    }
                    ...
                    _.each(methodNames, function(name) {
                        I[name] = function() {
                            ...
                            // 调用填充完成后的实例的对应方法。
                            if (fn) {
                                instance.handler[name].apply(instance.handler, arguments);
                            }

                            // 将调用信息附加至队列。                              
                            else {
                                instance.invokes.push({ name: name, args: arguments });
                            }
            ...
            flesh: function(/*String*/ klassName, /*OPTIONAL Function*/ constructor, /*OPTIONAL Object*/ methods) {
                ...
                instances[klassName].forEach(function(instance) {                   
                    ...
                    instance.invokes.forEach(function(invoke) {
                        ...
                        if (handler[name]) {
                            handler[name].apply(handler, invoke.args);
                        }
                    });
    ...
    var _MapWS = function foo(type, params, onsuccess, onerror, options) {
        ...
        _.find(foo._VENDOR_EX, function(EX) {
            VENDOR_EX = EX;
            method = VENDOR_EX.method ? VENDOR_EX.method(type, params, options) : VENDOR_EX.METHODS[type];
            return method;
        });
        ...
        var 
            ...
            ajaxOnerror = function(xhr, errorType) {
                if (onerror) {
                    onerror(I_ERROR.Ajax(errorType));
                }
            };
        ...
        //{
            ...
            _.each(params, function(value, name) {
                ...
                // Find-and-Execute
                if (fn) {
                    _.extend(data, fn.call(VENDOR_EX, value));
                }
        ...
        CTRIP: {
            ...
            revert: {
                ...
                city : function(data) {
                    var city, entity;
                    if (data && data.CityEntities) {
                        while (entity = data.CityEntities.pop(), entity) {
                            city = new City({ 
                                id     : entity.CityID, 
                                name   : entity.CityName,
                                parent : city
        ...
        AMAP: {
            ...
            METHODS: {
                ...
                POIS_SEARCH_AROUND: {
                    ...
                    parser : function(data) {
                        var pois = [], raw, i = 0;
                        while (raw = data.pois[i++], raw) {
                            pois.push(this.revert.poi(raw));
                        }
                        return pois;
                    }
                },
            ...
            method: function(type, params, options) {
                if (type == 'POIS_SEARCH') {
                    if (params.city) {
                        type = 'POIS_SEARCH_LOCAL';
                    }
                    if (params.coordinate) {
                        type = 'POIS_SEARCH_AROUND';
                    }
                }               
            ...
            revert: {
                ...
                poi: function(raw) {
                    ...
                    // 坐标
                    if (raw.location) {
                        poiInfo.coordinate = new Coordinate(raw.location);
                    }
                ...
                regeocode2poi: function(geocode) {
                    if (geocode.pois && geocode.pois.length) {
                        return this.poi(geocode.pois[0]);
                    }
                },
    ...
    var I_Set = window.Set ? window.Set : function() {
        var I = this, storage = [];
    ...
    Object.defineProperties(I_Timer.prototype, {
    ...
    var I_WeakMap = window.WeakMap ? window.WeakMap : function() {
            ...
            [
                ...
                function(info) {
                    ...
                    _.each(info, function(name, scale) {
                        if (name) {
                            addresses.push(new Address(window.parseInt(scale), name));
                        }
                    });
                    ...
                    for (var i = addresses.length - 1; i > 0; i--) {
                        addresses[i-1].parent = addresses[i];
                    }
    ...
    _.extend(Address.prototype, {
         ...
         */
         get: function(what) {
            ...
            do {                
                if (address.scale == what) {
                    return returns ? address.name : address;
                }
            } while (address = address.parent, address);
        ...
        accurateThan: function(address) {
            ...
            // 先判断精确程度（前提是双方都定义了标尺）
            if (I.scale && U.scale && I.scale < U.scale) {
                return true;
            }
    ...
    var I_Map_CTRIP = function(options) {
    ...
    var _Map_AMAP = (function() {
        ...
        return _Klass.create(
            ...
            function(callback) {
                ...
                _ME.loadJS('//webapi.amap.com/maps?v=1.3&key=' + _VENDOR.AMAP.KEY + '&callback=' + funcname);
            },
    ...
    var Map = function(options) {
        ...
        if (!options.center) {
            ...
            locate('coordinate', function(coordinate) {
                if (!touched) {
                    map.moveTo(coordinate).zoomTo(_zoom);
                }
            });
        }
    ...
    function MapMark() {
        ...
        var DEFs = [
            ...
            [
                ...
                function(options) {
                    if (options.poi) {
                        this.coordinate = options.poi.coordinate;
                        this.title = options.poi.name;
                    }
                ...
                function(options) {
                    var 
                        formatXY = function(xy) {
                            ...
                            if (_.isString(xy)) {
                                xy = xy.split(/\s|,/);
                            } else if (xy.constructor == Object) {
                                xy = [ xy.x || xy.width, xy.y || xy.height ];
                            }
                    ...
                    if (image.size) {
                        css += ';-size:' + formatXY(image.size).join(' ');
                    }
                    if (image.offset) {
                        css += ';-position:' + formatXY(image.offset).join(' ');
                    }
    ...
    function SMap(options) {
        ...
        if (marks) {
            for (var i = 0; i < marks.length; i++)  {
                marks[i] = new MapMark(marks[i]);
            }
        }

        if ( Lizard.app.vendor.is('CTRIP') && Lizard.app.code.is('MASTER') ) {
            map = new I_Map_CTRIP(options);
        } else {

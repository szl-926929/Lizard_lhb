...
define([
  ...
  var Calendar = function(options) {
    ...
    // 今天时间
    this._today = new Date(serverDate.getTime() + this.options.jetLag * 1000).setHours(0, 0, 0, 0);
    this.render();
  };
  ...
  Calendar.prototype = {
    ...
    getDateDesc: function(date) {
      ...
      } else if(dateTime - today === oneDay) {
        txt = '明天';
        todayCls = this.options.todayCls;
      } else if (dateTime - today === 2 * oneDay) {
        txt = '后天';
        todayCls = this.options.todayCls;
      }

 ...
 * 从cUtility中分离出来的定位组件
 */
define(['cBase', 'cUtility', 'cWidgetFactory', 'cStore', 'cHybridFacade', 'cUtilityPerformance', 'cGeoHelper', 'cModel'], function (cBase, Util, WidgetFactory, cStore, Facade, Performance, CtripGeoHelper, cModel) {
  "use strict";
  ...
  var cityModel = cBase.Class(cModel, {
    __propertys__: function() {
      this.url = 'https://m.ctrip.com/restapi/soa2/10398/json/LBSLocateCity';
      this.param = {};
    }
  }).getInstance();
  ...
  Geolocation.requestAMapAddress = function (lng, lat, callback, error, timeout) {
    ...
    var url = (ret == 1) ? ('//restapi.amap.com/v3/geocode/regeo?' + $.param({
        'location' : region,
        'key' : KEY,
        'radius' : 0,
        'extensions' : 'all'
  ...
  Geolocation.requestAMapKeyword = function (keywords, city, callback, error, timeout) {
    ...
    $.ajax({
      url : "//restapi.amap.com/v3/place/text?" + param,
      dataType : 'jsonp',
      success : function (data) {
        cperformance.groupEnd(uuidGeoService);
        var pois = (data && data.pois) || [];
  ...
  Geolocation.requestCityInfo = function (callback, error, posCallback, posError, isAccurate, cityCallBack, cityErrorCallBack) {
    ...
    for (var i in uuidGeoRequest) {
      cperformance.group(uuidGeoRequest[i], {
        name : 'GeoRequest',
        url : (Util.isInApp() ? 'Native' : 'Web') + ' function ' + i
      });
    }

    var _HybridLocate = function () {

      //-6……2014-09-03……JIANGJing……需要记录两个时间，故须拆分
      // var uuidGeoService = cperformance.getUuid();
      // cperformance.group(uuidGeoService, {
      //   name : "GeoRequest",
      //   url : "Native function",
    ...
    var WebMap = function (config) {
      ...
      function createMarker(params, mapObj, isDomestic) {
        ...
        function createSingleMarekr(params, mapObj, isDomestic) {
          ...
          if (!isDomestic) {
            $('<IMG/>').attr({
              src : 'https://res.m.ctrip.com/html5/Content/images/map_address.png'

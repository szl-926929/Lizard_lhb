  ...
  function (libs, cBase, AbstractStore, cUtility, CommonStore, baseModel, cStorage, pathUtil) {
      ...
      var AbstractModel = new cBase.Class(baseModel, {
          ...
          */
          ...
          buildurl: function () {
              if (cUtility.validate.isUrl(this.url)) {
                  return this.url.replace(pathUtil.parseUrl(this.url).hostname, this.baseurl.domain);
              } else {
          ...
          */
          excute: function (onComplete, onError, ajaxOnly, scope, onAbort) {
              ...
              if (!cache || this.ajaxOnly || ajaxOnly) {
                  ...
                  return this.execute(onComplete, onError, scope, onAbort, params)
          ...
          */
          ...
          auth = "", success = false,overdue = false;
              if (fromSOA && head) {
	              var ack = head.Ack;
	              //酒店模块报错ack返回值是1

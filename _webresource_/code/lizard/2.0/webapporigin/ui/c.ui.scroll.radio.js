...
define(['libs', 'cBase', 'cUILayer', 'cUIScrollList'], function (libs, cBase, Layer, ScrollList) {
  ...
  _attributes.onShow = function () {
    ...
    if ((navigator.userAgent.indexOf('MSIE')) != -1) {
      ...
      for (var i = 0, len = this.data.length; i < len; i++) {
        ...
        var len1 = _data.length;
        var _index = parseInt(this.index[i]);
        if (!_.isNumber(_index) && !_.isNaN(_index)) _index = 0;

        var _div = $('<div style="height: ' + (this.disItemNum * 44) + 'px; overflow: auto; position: relative;" class="cui-roller-bd ' + ((len == 3 && i == 0) ? 'cui-flex2' : '') + ' "><ul class="ul-list" style="height: auto;"></ul></div>');
        var _ul = _div.find('ul');

        for (var j = 0; j < len1; j++) {
          var _item = _data[j];
          _item.index = j;
          var _li = $('<li data-index="' + j + '" class="' + (_index === j ? 'current' : '') + '"  >' + _item.val + '</li>');
          _li.css('')
          _ul.append(_li);
        }
        this.wrapper.append(_div);
        _div[0].scrollTop = _index * 44;
  ...
  options.__propertys__ = function () {
    ...
    if ((navigator.userAgent.indexOf('MSIE')) != -1) {
      ...
      this.eventArr = {
        'click .ul-list li': function (e) {
          var el = $(e.currentTarget);
          var p = el.parent();
          p.find('li').removeClass('current');
          el.addClass('current');
        },
        'click .cui-btns-sure': function () {
          var scope = this;
          var arr = this.root.find('.ul-list').find('.current');
          var item = [];
          for (var i = 0, len = arr.length; i < len; i++) {
            var index = $(arr[i]).attr('data-index');
            item.push(this.data[i][index]);
          }
          scope.okClick.call(scope, item); //改变则触发事件
          scope.hide();
        },
        'click .cui-btns-cancel': function () {
          var scope = this;
          var arr = this.root.find('.ul-list').find('.current');
          var item = [];
          for (var i = 0, len = arr.length; i < len; i++) {

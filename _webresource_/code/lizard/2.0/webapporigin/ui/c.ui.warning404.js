...
define(['libs', 'cBase', 'cUIPageview', 'cWidgetFactory', 'cWidgetGuider'], function (libs, cBase, PageView, WidgetFactory) {
  ...
  options.setTmpl = function(tmpl, retryClass) {this.templateStr = tmpl, this.retryClass = retryClass}
  options.options = function(options) {
    options = _.extend({}, defaults, options || {});  

    this.root.html(_.template(this.templateStr || template, options));

    this.addClass('head-warning-top');
    this.retryDom = this.root.find(this.retryClass || '.cui-btns-retry');

...
define(['libs', 'cBase'], function (libs, cBase) {
  ...
  */
  options.start = function () {
    if (window.Lizard)
    {
      if (Lizard.hashObserveCount)
        Lizard.hashObserveCount++;
      else 
        Lizard.hashObserveCount =  1;
  ...
  */
  options.end = function () {    
    ...
    if (Lizard.hashObserveCount)
        Lizard.hashObserveCount--;
    if (window.Lizard && !Lizard.hashObserveCount)
      setTimeout(function(){Lizard.stopStateWatch = false;}, 200);      
  };

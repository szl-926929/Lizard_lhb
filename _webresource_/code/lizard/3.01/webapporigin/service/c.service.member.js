...
define([(Lizard.app.vendor.is('CTRIP') || Lizard.isHybrid) ? 'cHybridMember' : 'cWebMember', 'cUserStore'], function (Member, UserStore) {
  ...
  /**
   * 判断当前用户是否登录,包括会员以及非会员
   * @method cMemberService.isLoginWithoutType
   * @returns {Object|boolean} isLoginWithoutType 当前用户是否登录
   * @see UserStore#isLogin
   */
  Member.isLoginWithoutType = function(callback){return userStore.isLoginWithoutType(callback);};
  /**
   * 判断当前用户是否登录

 ...
 * @version V1.0
 */
define(['cCoreInherit', 'cAjax', 'cUtilCommon', 'cUtilPath','cMessageCenter', 'cUtilObject'], function (cCoreInherit, cAjax, cUtilCommon, pathUtil,MessageCenter, cUtilObject) {
/**
...
//      }
    ...
    pushValidates: function (handler) {
      if (typeof handler === 'function') {
        this.validates.push(_.bind(handler, this));
      }
    },
    /**
    ...
    getParam: function () {
      ...
      if(_.isObject(this.param)){  //model post请求参数只能为对象
        params = cUtilObject.extend(true,{},this.param);
      }
      return params || {};
    },
    ...
    _execute: function (onComplete, onError, scope, onAbort, params) {
      ...
      var __onComplete = _.bind(function (data) {
        //保存服务请求日志
        ...
        if (this.validates && this.validates.length > 0) {
          ...
          for (var i = 0, len = this.validates.length; i < len; i++) {
            ...
            if ((typeof validates === 'boolean')) {
              ...
              if(validates && validates.verification){
                verificationCode();
                return;
              }
            }
        ...
        function verificationCode(){
          //此处需要添加弹出验证码的框
          MessageCenter.subscribe('verificationCode', _.bind(function(){
      ...
      var __onError = _.bind(function (e) {
        if (self.isAbort) {
          self.isAbort = false;
      ...
      if(cUtilCommon.isUseSOTPSendHTTPRequest() && Lizard.app.vendor.is('CTRIP') && Lizard.isHybrid && cUtilCommon.isSOA2Request(url)){ // jshint ignore:line
        var headers = {
        ...
        //Pragma:no-cache
        //Referer:http://m.ctrip.com/webapp/hotel/
        //目前只有满足以上三种条件，才会使用http改成stop的处理
        require(["cHybridShell"],function(cHybridShell){
          var fn = new cHybridShell.Fn('sotp_send_http_requst',function(data,err){
            if(err){
              __onError(err);
              return;
            }
            try{
              ...
              if(resBody){
                //http请求返回值
                __onComplete(JSON.parse(resBody));
              }else{
                __onError();
              }
            }catch(e){
          ...
          fn.run(url,this.method || "POST",headers,reqParams);
        });

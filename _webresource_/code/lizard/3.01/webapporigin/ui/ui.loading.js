define([Lizard.config.loadingtmpl || getAppUITemplatePath('ui.loading'), "cUtilEvent"], function (template, EventUtil) {
  function Loading() {
    this.template = _.template(template);
  }
  Loading.prototype = {
    createMask: function () {
      this.mask = document.querySelector('cui-mask');
      if (this.mask) {
        return;
      }
      this.mask = document.createElement('div');
      this.mask.className = "view cui-mask";
      this.mask.setAttribute("style", "width: 100%; position: absolute; left: 0px; top: 0px; z-index: 3001; display: none");
      this.mask.style.height = document.body.clientHeight + 'px';
      document.body.appendChild(this.mask);
    },
      
    show : function (data, template) {
      if (!this.mask) {
        this.createMask();
      }
      ...
      this.el = document.body.appendChild(el.firstChild);
      if (this.el.querySelector('.loading-layer2-before')) {
        this.draw(this.el.querySelector('.loading-layer2-before'));  
      }      
    },

define(['cPageModelProcessor', 'cMessageCenter', 'cUtilPerformance', 'cPageParser'], function (callModels, MessageCenter, cperformance) {
  ...
  callModels(pageConfig, function (datas, pageConfig) {
    ...
    renderNode.innerHTML = renderObj.viewport;
    var container = document.getElementById('main').querySelector('.main-viewport');
    if (renderNode.firstChild) {
      if (container.firstChild) {
        renderNode.firstChild.insertBefore(document.getElementById('main').querySelector('.main-viewport').firstChild);
      } else {
        container.appendChild(renderNode.firstChild);
      }
    }
  }, function (datas) {

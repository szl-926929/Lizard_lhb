 ...
 * @version V2.1
 */
define(['cCoreInherit', 'cLocalStore', 'cCommonStore', 'cParserUtil', 'react'], function(cCoreInherit, cStore, CommonStore, ParseUtil, React){
  ...
  {    
    var configStr = pageDocNode.querySelector('script[type="text/lizard-config"]').innerHTML;
    if (!configStr)
    {
  ...
  */
  function getPageTemplates(){    
    var ret={}, templates = pageDocNode.querySelectorAll('script[type="text/lizard-template"]');
    _.each(templates, function(template){      
      if (template.getAttribute('id')){
        ret[template.getAttribute('id')]={
          'runat': template.getAttribute('runat')||'all',
          'text': removeTags(template, 'client')
        };
      }
    });
    return ret;
  }
  function removeTags(tmplNode,runat){
    _.each(tmplNode.querySelectorAll('[runat=' + runat + ']'), function(subnode){
      subnode.parentNode.removeChild(subnode);
    });
    return tmplNode.innerHTML;
  }
  
  Lizard._initParser = function(url, html)
  {
    pageDocNode = document.createElement('div');
    pageDocNode.innerHTML = html;
  ...
  */
  ...
  function _setTDKInfo(datas, pageConfig)
  {
    ...
    var title = pageDocNode.querySelector('title');
    if (TDK.title) {
      if(title) {
    ...
    _.each(TDK, function(val, key){
      ...
      var metaNode = pageDocNode.querySelectorAll('meta[name="' + key + '"]');
      if (metaNode) {
        _.each(metaNode, function(node){
  ...
  function _setUBTInfo()
  {
    ...
    _.each(varNames,function(varName){
      var values = '';
      var node = pageDocNode.querySelector('#' + varName);
      if (node)
      {
        values = node.value;
      }
    ...
    if(_.isArray(result.ab_testing_tracker) ){
      ...
      _.each(result.ab_testing_tracker, function(val,key){
        _.each(val.getAttribute("value").split(";"),function(val1,key1){
          if(val1){
            ret.push(val1);
          }
        });
  ...
  Lizard.render = function(pageConfig, datas, renderAt)
  {
    ...
    if (_.isFunction(pageConfig.validate))
    {
      ...
      {
        return pageConfig.modelOnError(datas);
      }         
    }
    //else
    //{

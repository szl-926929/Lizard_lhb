(function () {
  ...
  ****/
  ...
  window.app.callback = function (options) {
    function web_view_finished_load() {
      //多个Iframe APP会重复下发web_view_finished_load，这里作下简单保护
      if (Lizard.instance) {
        return;
      }
    ...
    var methods = {
      'web_view_finished_load': function () {
        //酒店怪异问题，通过延迟初始化可以解决，这里通过配置项判断
        if (Lizard.config.delayExecFinishedLoad) {
          setTimeout(web_view_finished_load, Lizard.config.delayExecFinishedLoad);
        } else {
      ...
      'init_member_H5_info': function (params) {
        ...
        require(['libs', 'cCommonStore'],
          function (libs, CommonStore) {
            ...
            if (Lizard.config.isBeforeViewFinishedLoadInitApp || Lizard.config.isViewFinishedLoadInitApp) {
              require(['cGuiderService'], function(Guider){
                Guider.create(); 
              });             
            }
          });

  ...
  function _sendReq(opt) {
    ...
    var obj = {
      ...
      success: function (res, status, xhr) {
        ...
        cperformance.performance[uuidXhrSend].info = ['code:', xhr.status].join();
      ...
      error: function (err, status) {
        if (status !== 'abort') {
          ...
          if (err && _.isObject(err)) {
            cperformance.performance[uuidXhrSend].info = ['code:', err.status, '    res:', err.responseText].join();
          }
          cperformance.groupEnd(uuidXhrSend);
        }
        if (opt.error) {

  ...
  var ImgLazyload = _.inherit({
    ...
    checkWrapperDisplay: function () {
      ...
      if ($(this.imgs[0]).offset().width === 0) {
        this.isError = true;
        var maxTimes = 0;
        this.TIMERRES = setInterval(_.bind(function () {
          maxTimes ++ ;
    ...
    bindEvents: function () {
      ...
      //为container绑定事件
      var scrollTimeout;
      this.container.on('scroll.imglazyload' + this.uuid, _.bind(function () {
        if(scrollTimeout){
          clearTimeout(scrollTimeout);
        }
        scrollTimeout = setTimeout(_.bind(function(){
          if(scrollTimeout){
            clearTimeout(scrollTimeout);
          }
          this.lazyLoad();
      ...
      $(window).on('resize.imglazyload' + this.uuid, _.bind(function () {
        this.initImgContainer();
      }, this));

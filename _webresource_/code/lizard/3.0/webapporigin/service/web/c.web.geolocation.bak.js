...
define(['cGeoHelper', 'cUtilPerformance', 'cModel', 'cCoreInherit'], function (cGeoHelper, cperformance, cModel, cCoreInherit) {
  ...
  var cityModel = cCoreInherit.Class(cModel, {
    __propertys__: function() {
      this.url = '//m.ctrip.com/restapi/soa2/10398/json/LBSLocateCity';
    }
  }).getInstance();
  ...
  Geo.showMapWithPOI = function (options) {
    ...
    var webMap = function (config) {
      ...
      if (config.zoomCallBack) {
        if(!isOverseasAndGoogle) { AMap.event.addListener(mapContainer, 'zoomchange', function () {
          var newMarkers = config.zoomCallBack && config.zoomCallBack(this.getZoom());
          if (newMarkers) {
            this.clearMap();
            createMarker(newMarkers, this, isOverseasAndGoogle);
          }
        });} else{ google.maps.event.addListener(mapContainer, 'zoom_changed', function () {
          var newMarkers = config.zoomCallBack && config.zoomCallBack(this.getZoom());
          if (newMarkers) {
            //删除原有的markers
            if (marekrViews.marekrs && marekrViews.marekrs.length > 0) {
              for (var i = 0; i < marekrViews.marekrs.length; i++) {
                if(marekrViews.overlays[i].setMap){marekrViews.overlays[i].setMap(null);}
                if(marekrViews.marekrs[i].setMap){marekrViews.marekrs[i].setMap(null);}
              }
              marekrViews = {
                marekrs : [],
                overlays : []
      ...
      function createMarker(params, mapObj, isDomestic) {
        ...
        function createSingleMarekr(params, mapObj, isDomestic) {
          ...
          if (!isDomestic) {
            $('<IMG/>').attr({
              src : '//res.m.ctrip.com/html5/Content/images/map_address.png'
...
//                              });
            ...
            marker = new google.maps.Marker({
              ...
              map : mapObj,
              icon : "//res.m.ctrip.com/html5/Content/images/map_address.png",
              id : params.markerId
            });
            //如果id存在设置为中心点
            if (params.id) {
  ...
  Geo.requestCityInfo = function (callback, error, posCallback, posError, isAccurate, cityCallBack, cityErrorCallBack) {
    ...
    var successCallback = function (pos) {
      ...
      if(posCallback){ posCallback(lng, lat);}
  ...
  Geo.requestGeographic = function (callback, error, timeout) {
    ...
    var loadScript = function (url, inCallback, isWrite) {
      var script = document.createElement("script"),
        dw = document.write;// jshint ignore:line
      ...
      if (script.readyState) {
        script.onreadystatechange = function () {
          if (script.readyState == "loaded" ||
            ...
            document.write = dw;// jshint ignore:line
            if (isWrite&&inCallback) {
              inCallback();
            }
          }
        };
      ...
      script.src = url;
      document.write = function (ad) {  // jshint ignore:line
  ...
  Geo.tansformLongitude = function (lng, lat, callback, error, timeout) {
    ...
    cperformance.group(uuidGeoService, {
      name : "GeoRequest",
      url : "//restapi.amap.com/v3/assistant/coordinate/convert",
      type : "AMap service"
    });
    ...
    $.ajax({
      url : "//restapi.amap.com/v3/assistant/coordinate/convert?" + param,
      dataType : 'jsonp',
      success : function (data) {
        cperformance.groupEnd(uuidGeoService);
        if (data && data.status === '1') {
          var l = data.locations.split(',');
          if(callback){callback(l[0], l[1]);}
        } else {
          if(error){error();}
        }
      },
  ...
  Geo.requestAMapAddress = function (lng, lat, callback, error, timeout) {
    ...
    var urls =  (ret == 1) ? [('//restapi.amap.com/v3/geocode/regeo?' + $.param({
      'location' : region,
      'key' : KEY,
      'radius' : 0,
      'extensions' : 'all'

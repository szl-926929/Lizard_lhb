 ...
 * @name UIScroll
 * @example
 * var scroll = new UIScroll({
 *   scrollType: 'x',
 *   scrollbars: false,
 *   wrapper: this.$('.js-wrapper')
 *   scroller: this.$('.js-scroller');
 * });
 */
define([], function () {
  ...
  function IScroll(opts) {
    ...
    if (!opts.wrapper[0] || !opts.scroller[0]){
    	  throw 'param error';
    }
  ...
  IScroll.prototype = {
    ...
    refresh: function () {
      ...
      if (this.options.scrollType == 'y') {
        ...
        if (this.options.scrollOffset === 0) {
          this.maxScrollY = Math.min(this.maxScrollY, 0);
        }
      } else {
        this.maxScrollY = 0;
        if (this.options.scrollOffset === 0) {
          this.maxScrollX = Math.min(this.maxScrollX, 0);
        }
      }
    ...
    _end: function (e) {
      ...
      if (this.options.step) {
        ...
        if ((this.options.scrollType == 'x' && tdistanceX < 50) || (this.options.scrollType == 'y' && tdistanceY < 50)){
        	   time = 100;
        }
    ...
    _transitionTimingFunction: function (easing) {
	  ...
	  if(this.indicator){
	  	this.indicator.transitionTimingFunction(easing);
	  }
    },
    ...
    _transitionTime: function (time) {
      ...
      if(this.indicator){
      	this.indicator.transitionTime(time);
      }

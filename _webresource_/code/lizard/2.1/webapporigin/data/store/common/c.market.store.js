 ...
 */
define(['cCoreInherit', 'cLocalStore', 'cLocalStorage','cUtilDate'], function (cCoreInherit, cLocalStore, cLocalStorage,CDate) {
  var Common = {};
  ...
  Common.UnionStore = new cCoreInherit.Class(cLocalStore, {
     ...
     */
    set:function(value){
      var savedate = (new CDate()).format('Y/m/d H:i:s');
      var timeout = this._getNowTime();
      timeout = timeout.addSeconds(this._getLifeTime()).format('Y/m/d H:i:s');
      var obj = {
        value:    value,
        data : value,
        timeout:  timeout,
      ...
      if(this.sProxy && this.sProxy.proxy && this.sProxy.proxy.setItem){
        this.sProxy.proxy.setItem(this.key,JSON.stringify(obj));
      }
    }
  });

    ...
    set: function (data) {
      ...
      if (data.back !== false) {
        if (_.isObject(data.back)) {
          ...
          if (_.isFunction(data.events.returnHandler)){
      ...
      if (data.left){
      	_data.left = data.left.concat(_data.left);
      }
      if (data.right){
      	_data.right = data.right.concat(_data.right);
      }
      ...
      //如果外部设置了center直接替换
      if (_.isObject(data.center)){
      	_data.center = data.center;
      }

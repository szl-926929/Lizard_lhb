...
(function () {
  ...
  function loadRes() {
    ...
    if (Lizard.notpackaged) {
      ...
      if (!Lizard.app.code.is('MASTER') && !Lizard.app.code.is('YOUTH') && !Lizard.isHybrid) {
        var isDebug = window.location.href.indexOf("debug=1") !== -1;
        if(isDebug){
          //只有在测试环境才加载lizard.web.js
          basescripts.push(Lizard.uibeta_sandbox?Lizard.dir + 'lizard.web.beta.js':Lizard.dir + 'lizard.web.src.js');
        }
      }
  ...
  if (Lizard.app.code.is('TV') || Lizard.app.code.is('TY')) {
    window.app = {};
    window.app.callback = function (options) {
      var methods = {
        'web_view_finished_load_lite' : function (params) {
          //在此处将clientId以及SystemCode写到localstorage中
          var amdArr = ['cCommonStore','cMessageCenter'];
          require(amdArr, function (CommonStore,MessageCenter) {
        ...
        'init_member_H5_info_tieyou':function(params){
          var amdArr = ['cCommonStore'];
          require(amdArr, function (CommonStore) {
            var wStore = window.localStorage;
  ...
  //!(Lizard.isHybrid) && Lizard.app.vendor.is('CTRIP') && !(_.find($('SCRIPT'), function(script){return $(script).attr('src').indexOf('lizard.hybrid.js') > -1;})) && document.write('<script src="' + Lizard.dir + 'lizard.hybrid.js"><\/script>');// jshint ignore:line
})();